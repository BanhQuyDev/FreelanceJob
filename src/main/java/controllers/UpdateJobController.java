            ...
            switch (action) {
                ...
                case "Update Post":
                    ...
                    if (status.equals("Posted")) {
                        boolean check_update = jDao.updateJob(updateJob, "2");
                        if(check_update) {
                        ...
                        boolean check_update = jDao.updateJob(updateJob, "3");
                        if(check_update) {
                            boolean check_delete_old_skill = jDao.deleteJobSkill(id_job);
                            boolean check_new_skill = jDao.createSkillJob(skillList, id_job);
                            String content = title + " was updated by " + user.getName();
                            NotificationDAO notiDao = new NotificationDAO();
                            NotificationDTO notiDto = new NotificationDTO(user.getId(), content, 1);
                            boolean check_noti = notiDao.createNotification(notiDto);
