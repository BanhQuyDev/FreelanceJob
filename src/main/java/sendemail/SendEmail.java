...
public class SendEmail {
    ...
    public static boolean sendEmailBanUser(UserDTO user, String reason) throws MessagingException {
        boolean check = false;
        Properties pr = new Properties();
        pr.put("mail.smtp.auth", "true");
        pr.put("mail.smtp.starttls.enable", "true");
        ...
        Message message = prepareMessageBanUser(session, myAccountEmail, user, reason);
        Transport.send(message);
        check = true;
        return check;
    }
    
    public static boolean sendEmailUnbanUser(UserDTO user) throws MessagingException {
        boolean check = false;
        Properties pr = new Properties();
        pr.put("mail.smtp.auth", "true");
        pr.put("mail.smtp.starttls.enable", "true");
        ...
        Message message = prepareMessageUnbanUser(session, myAccountEmail, user);
        Transport.send(message);
        check = true;
        return check;
    }
    ...
    private static Message prepareMessageBanUser(Session session, String myAccountEmail, UserDTO user, String reason) {
        try {
            Message message = new MimeMessage(session);
            message.setFrom(new InternetAddress(myAccountEmail, "FPT Freelance Job Team"));
            message.setRecipient(Message.RecipientType.TO, new InternetAddress(user.getEmail()));
            message.setSubject("ACCOUNT DEACTIVATION ANNOUCEMENT");
            String mess = "<!DOCTYPE html>\n"
                    + "<html lang=\"en\">\n"
                    + "\n"
                    + "<head>\n"
    ...
    private static Message prepareMessageUnbanUser(Session session, String myAccountEmail, UserDTO user) {
        try {
            Message message = new MimeMessage(session);
            message.setFrom(new InternetAddress(myAccountEmail, "FPT Freelance Job Team"));
            message.setRecipient(Message.RecipientType.TO, new InternetAddress(user.getEmail()));
            message.setSubject("ACCOUNT REACTIVATED ANNOUCEMENT");
            String mess = "<!DOCTYPE html>\n"
                    + "<html lang=\"en\">\n"
                    + "\n"
                    + "<head>\n"
